...
title Application::getData(bool* flag) {
    title temp;
    std::cout << "\t";
...
void Application::departing(DepartingService* service, bool* stop) {
    while (true) {
...
void Application::arriving(ArrivingService* service, bool* stop) {
    while (true) {
        if (service->getSizeCheckInQueue() > 0) {
            service->checkInPassenger();
...
int Application::mainProgram() {
    ...
    std::thread depar(std::bind(&Application::departing, std::move(this), &departingService, &stop));
    std::thread arrived(std::bind(&Application::arriving, std::move(this), &arrivingService, &stop));
